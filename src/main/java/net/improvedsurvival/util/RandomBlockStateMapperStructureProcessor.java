...
public class RandomBlockStateMapperStructureProcessor extends StructureProcessor {
	...
	public RandomBlockStateMapperStructureProcessor(BlockState sourceState, BlockState targetState, float integrity) {
		this(sourceState, targetState, integrity, sourceState);
	}
	
	public RandomBlockStateMapperStructureProcessor(BlockState sourceState, BlockState targetState, float integrity, BlockState backupState) {
	...
	public RandomBlockStateMapperStructureProcessor(Dynamic<?> dynamic) {
		this(
				BlockState.deserialize(new Dynamic<>(JsonOps.INSTANCE, gson.fromJson(dynamic.get("source_state").asString().get(), JsonElement.class))),
				BlockState.deserialize(new Dynamic<>(JsonOps.INSTANCE, gson.fromJson(dynamic.get("target_state").asString().get(), JsonElement.class))),
				dynamic.get("probability").asFloat(1.0f),
				BlockState.deserialize(new Dynamic<>(JsonOps.INSTANCE, gson.fromJson(dynamic.get("backup_state").asString().get(), JsonElement.class)))
		);
	}
